...
function App() {
  ...
  /* EXAMPLE 2 */
  /* reference HTML elements on the page with useRef */
  // use useRef to reference the name input field
  // use element type and initial value
  // on the input field the "ref" property will be set: ref={nameInput}
  const nameInput = useRef<HTMLInputElement>(null);

  // create focus() callback function for "Click on me to focus on name field" button
  // use nameInput reference to identify the element to focus on when button is clicked
  const focus = () => {
  ...
  return (
    <Container>
      ...
      <StyledSection>
        ...
        <button type="submit" onClick={(event) => handleNameInput(event)}>
          Submit
        </button>
        <div>
          Your current name is <Name>{name}</Name> but you used to be called{" "}
          <PrevName>{prevName.current}</PrevName>.
        </div>
      </StyledSection>
      <StyledSection>
